...
class AppController extends Controller
{
    ...
    public function _afterLogin($isRedirect = true)
    {
        if ($this->Auth->isAuthorized()) {
            ...
            // Check if user has any course enrollment (skip for super admin and admin)
            if (!User::hasPermission('controllers/users')) {
                $enrolledCourses = $this->User->getEnrolledCourses(User::get('id'));
                $instructorCourses = $this->User->getInstructorCourses(User::get('id'));
                $tutorCourses = $this->User->getTutorCourses(User::get('id'));
                
                if (empty($enrolledCourses) && empty($instructorCourses) && empty($tutorCourses)) {
                    $this->Auth->logout();
                    
                    // Prepare message and logout URL
                    $message = __('You do not have any course enrollment in iPeer. Please contact your administrator.', true);
                    $samlLogoutUrl = Configure::read('SAML_LOGOUT_URL');
                    $logoutUrl = !empty($samlLogoutUrl) ? $samlLogoutUrl : '/logout';
                    
                    // Display alert and redirect to SAML logout
