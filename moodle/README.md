...
NOTE: This chart is based on https://github.com/helm/charts/tree/master/stable/moodle

## TL;DR;

```console
$ helm install ctlt/moodle
```

## Introduction

This chart bootstraps a [Moodle](https://github.com/ubc/moodle-docker) deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
```console
$ helm install --name my-release ctlt/moodle
```
...
The above parameters map to the env variables defined in [ctlt/moodle](http://github.com/ubc/moodle-docker). For more information please refer to the [ctlt/moodle](http://github.com/ubc/moodle-docker) image documentation.
...
```console
$ helm install --name my-release -f values.yaml ctlt/moodle
```
...
```console
$ helm install --name my-release \
  --set ingress.enabled=True,ingress.hosts[0]=moodle.domain.com,serviceType=ClusterIP,moodleUsername=admin,moodlePassword=password,mariadb.mariadbRootPassword=secretpassword ctlt/moodle
```
...
The [Moodle](https://github.com/ubc/moodle-docker) image stores the Moodle data and configurations at the `/moodledata` path of the container.
